...
class (HasLinearMap v, Monoid (Render b v)) => Backend b v where
  ...
  --   information.  It returns a modified options record, the
  --   /inverse/ of any transformation applied to the diagram (which
  --   can be used to transform /e.g./ screen coordinates back into
  --   local diagram coordinates), and the adjusted diagram itself.
  --
  ...
  -- | Render a diagram in a way specific to this backend.
  ...
  renderDiaT b opts d = (t, doRender b opts' . renderData b $ d')
